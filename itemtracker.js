...
function print_response(data) {
	...
	html.append('<table width=100% height=100px id=stuff></table');
	$.each(data, function(x, contents) {
		...
		$.each(contents.item_list, function(y, object){
			...
			html.find('#items').children('td').append('<img width=48px height=48px src=collectibles/collectibles_'+("000" + object.item_id).slice(-3) + '.png />');
		});
